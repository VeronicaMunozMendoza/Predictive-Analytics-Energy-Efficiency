#### This script provides starter code and comments for your assignment.
#
# Copyright: Technological University of the Shannon, 2022
# Author: jonny.odwyer@tus.ie
...
#### Your Data Understanding Code Goes Here
...
par(mfrow = c(2, 4)) # setting the plots in a 2x4 grid.

for(col in names(train)){
  if(is.numeric(train[[col]])){
    boxplot(train[[col]], ylab="Y", main=col)
  }
}
...
# EXPLORE STANDARISED DATA
library(dplyr)

standarised_vals <- train %>% mutate_all(~(scale(.) %>% as.vector())) # standardizing values.

cor(standarised_vals) # checking the correlation coefficient to see the relationship between variables.

standarised_data <- standarised_vals[, c(1,2,3,4,5,6,7,8,9)] # Creating a new object with all the independent variables. 

str(standarised_data)  #  Checking the structure/format of the data.

pairs.panels(standarised_data [,c("Relative_Compactness", "Surface_Area", "Wall_Area",   # creating a scatter plot matrix.
                       "Roof_Area", "Overall_Height", "Orientation",           
                       "Glazing_Area", "Glazing_Area_Distribution", "Y")], 
                         main = "Scatterplot Matrix of Standarised Data")


# CONCLUSIONS: As per the scatter plot matrix I do not see much of a difference between the standardized to the non-standarised data. 
# Thus I'll be using the Non-Standardized data for my model. 
...
#### Your Final Evaluation Code Goes Here
